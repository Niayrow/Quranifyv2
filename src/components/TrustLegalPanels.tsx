...
const Em: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <span className="text-[#f0d1bc]">{children}</span>
);

/** Mot super important — cuivre gras */
const Strong: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <strong className="font-bold text-[#f1d4c1]">{children}</strong>
);
...
const LinkOut: React.FC<{ href: string; children: React.ReactNode }> = ({ href, children }) => (
  <a
    ...
    className="inline-flex items-center gap-1 font-bold text-[#f1d4c1] underline-offset-2 hover:underline"
  >
    {children}
  ...
  <article className="flex flex-col gap-8 pb-10">
    ...
    <Section icon={BookOpen} title="Source audio principale">
      ...
      <p>
        ...
        (ex. <code className="rounded bg-[#132031]/80 px-1.5 py-0.5 font-mono text-[12px] font-bold text-[#f1d4c1]">serverN.mp3quran.net</code>).
      </p>
    </Section>
...
export const TermsPanel: React.FC = () => (
  <article className="flex flex-col gap-8 pb-10">
    ...
    <Section icon={BookOpen} title="Usage attendu">
      <ul className="m-0 flex list-disc flex-col gap-2 pl-4 marker:text-[#cea687]">
